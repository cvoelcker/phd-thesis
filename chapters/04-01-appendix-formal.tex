...
\begin{proof}
    Let $e$ be an eigenvector of $A$ and $\lambda$ the corresponding eigenvalue. Then $$(I - \gamma A)^{-1}e = \sum_{n=0}^\infty \gamma^n A^ne = \sum_{n=0}^\infty (\gamma\lambda)^n e = \frac{1}{1 - \gamma\lambda}e.$$ As $(1-\gamma\lambda)^{-1}$ is a monotonic function for $-1 \leq \lambda \leq 1$ and $\gamma\in[0,1]$, the ordering of the eigenvalues remains the same.
\end{proof}
...
\begin{lemma}[Lemma 5 of \citealt{tang2022understanding}]\label{prop:understanding:TangResult2}
...
The notion of stability used in \textcite{ghosh2020representations} is that of convergence to the unique fixed point of the projected Bellman update of the linear ODE induced by the $L_\text{TD}$ loss when fixing $\Phi$. In the two-timescale scenario considered in \autoref{chap:understanding}, this corresponds to the ``inner'' ODE over $\hat{V}$.

\begin{lemma}[Proposition 3.1 of \citealt{ghosh2020representations}]\label{prop:understanding:gosh1}
...
\begin{lemma}[Proposition 3.2 of \citealt{ghosh2020representations}]\label{prop:understanding:gosh2}
...
\begin{lemma}[Theorem 4.1 of \citealt{ghosh2020representations}]\label{prop:understanding:gosh3}
...
\begin{lemma}[Corollary of \citealt{ghosh2020representations}]\label{lem:understanding:gosh:corr}
...
\begin{proof}
    ...
    By \autoref{ass:cvaml_mdp} there does not exist a $x\in\mathcal{X}$ so that $f(x) = \EEX{p}{f(x)}$. However, as $\mathrm{Var}_q[f(x)] = 0$, $\EEX{q}{f(x)} = f(x_i)$ for some $x_i\in\mathcal{X}$. Therefore, $\EEX{q}{f(x)} \neq \EEX{p}{f(x)}$, which concludes the proof.
\end{proof}
