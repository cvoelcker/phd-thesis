...
In this thesis, we set out to understand how we can improve value function learning in \ac{rl}, by incorporating decision-aware and general purpose techniques that improve learned representations and world models.
...
We started in \autoref{chap:overestimation} by understanding the central role that neural network representations play for stabilizing value learning in \ac{drl}.
As our initial experiments show, relying purely on a model-free Q learning loss to shape the representation learning in a neural network leads to instability and divergence.
...
To go beyond merely regularizing features, in \autoref{chap:understanding} we took a look at auxiliary tasks which use additional losses to shape the representation layers of a neural network.
...
We then took a look at \ac{mbrl} in \autoref{chap:vagram} and found a similar issue with decision-aware learning to that of value overestimation.
...
In \autoref{chap:cvaml}, we started combining the insights into model learning and representation learning.
We first introduced the family of $(m,b)$-VAML losses that encompasses both MuZero and IterVAML.
We showed that these losses lead to wrong model and value estimates when used with stochastic environment models and suggested a calibrated version of the losses that shows improved accuracy in model and value learning..
...
Finally, in \autoref{chap:mad}, we revisited the difficult high UTD training regime first discussed in \autoref{chap:overestimation} and used our gained knowledge to build a stable and efficient latent value-aware model learning algorithm  called MAD-TD. 
It is able to smoothly make progress on hard tasks in the standard mujoco locomotion benchmarking suite and does not require previously established workarounds to stabilize training such as resetting.
...
Extending gradient-aware model learning to account for first-order gradient estimation is an important topic for future work.

\subsection{Explicit and implicit uses of model learning}

In our work, we have shown explicit uses of world models to improve value function learning (\autoref{chap:vagram}), implicit uses of model-based losses to stabilize representation learning (\autoref{chap:understanding}), and combinations of both (\autoref{chap:cvaml} and \autoref{chap:mad}).
...
When discussing optimal ways of representing the information an agent gathers, a central question that we have not tackled so far is what kind of task an agent is attempting to learn in the first place.
...
As the theory and experiments in \autoref{chap:understanding} show, the structure of the environment is relevant for choosing proper representation learning methods, and the same is true for any other algorithmic design decisions as well.
...
We propose to bring decision-making environments themselves into the focus of the reinforcement learning literature as an area of research equal to the algorithms which are traditionally centered in the community.
This research should be structured around the goal of identifying formalizeable, testable, and impactful characteristics of environments.
...
% Preliminary work following up on \textcite{voelcker2024can} has shown that the instability of all methods presented in this thesis on the Hopper environment can largely be attributed to a complex, multi-modal start state distribution in this specific environment.
...
The most promising direction forward is therefore to see both as two ends of a spectrum and ask what algorithms are able to synthesize the best of both worlds.
We have shown that both finding the right losses that provide synergy with each other and using both, as well as combining losses of either type to create novel loss functions are viable strategies.
