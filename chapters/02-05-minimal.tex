...
Instead of solely relying on data gathered by a target policy, \emph{off-policy} \ac{rl} aims to leverage experience gathered by past policies \parencite{suttonbook} to fit the value function of a target policy. 
Ideally, given a fixed dataset of experiences, training over several iterations should help extract important information from past data.
However, as we have seen in \autoref{chap:overestimation}, simply increasing the number of gradient update steps, the \emph{replay ratio} or \emph{update-to-data (UTD) ratio}, can lead to highly unstable learning~\parencite{nikishin2022primacy,doro2023barrier,hussing2024dissecting,nauman2024bigger}.

Prior work has stabilized the learning by using double Q minimization to reduce overestimation \parencite{fujimoto2018addressing}, training ensemble methods to improve value estimation \parencite{chen2021randomized,hiraoka2022dropout}, introducing architectural regularization (\autoref{chap:overestimation}, \textcite{hussing2024dissecting,nauman2024bigger}), or resetting networks periodically throughout the learning process \parencite{doro2023barrier,schwarzer2023bigger,nauman2024bigger}.
However, while useful, pessimistic underestimation and architectural regularization are insufficient by themselves  to decisively combat the problem, and so most methods resort to either network resets or ensembles.
In our resetting experiments in \autoref{sec:overestimation:limitations_experiment} we see that in difficult tasks, resetting still improves policy performance.
...
However, these state-action pairs themselves are not in the replay buffer, as the actions are from the current policy, and hence their value estimate is not directly improved by training.
We call these \emph{unobserved on-p[olicy]} actions to signal that they are not included in the replay buffer (so \emph{unobserved} from the point of view of the training) yet stem from the current target policy.
...
\begin{align}
\mathcal{L}\left(\hat{Q}\middle|\blue{\mathcal{D}}, \pi\right) = \frac{1}{|\mathcal{D}|} \sum_{t=1}^T \left| \hat{Q}(\blue{x_t,a_t}) - \left[\blue{r_t} + \gamma \hat{Q}\left(\blue{x_{t+1}}, \red{a'}\right)\right]_\mathrm{sg} \right|^2\quad \mathrm{with }~\red{a'}\sim \pi(\cdot|\blue{x_{t+1}}) \enspace. \label{eq:mad:off_policy_q_update}
\end{align}
...
Hence, we require the Q value to generalize to potentially unseen actions.
...
In \autoref{chap:overestimation} we showed that this correlates with a strong increase in the norm of unnormalized neural network weights.
However, we did not describe a root cause of this overestimation.

We now conjecture that one of the major causes of the instability of high UTD off-policy learning are wrong Q values on \emph{unobserved actions}.
It is a well-known problem that off-policy TD learning is not guaranteed to converge, even when only considering state-value learning \parencite{baird1995residual,tsitsiklis1996analysis,sutton2016emphatic,ghosh2020representations}.
...
Furthermore, we assume that the feature matrix $\Phi$ which is constructed by concatenating all state-action features, has full rank.
...
We use $R \in \mathbb{R}^{X\cdot A}$ to denote the vector of rewards.
Finally, $D^{\pi} \in \mathbb{R}^{X\cdot A \times X\cdot A}$ is a matrix where the main diagonal contains the discounted state-action occupancies of $P^\pi$ starting from $\rho$.
If we assume access to a mixed replay buffer $\mathcal{D} = \bigcup \{\mathcal{D}^{\pi_1}, \dots, \mathcal{D}^{\pi_n}\}$ gathered with different policies, the population Q learning loss for a target policy $\Pi$ can be written as
\begin{align}
...
\begin{align}
    \dot\theta &= - \eta \Phi \underbrace{\sum_{i=1}^n D^{\pi_i} \left(I - \gamma P^\pi \right)}_{\mathrm{Key matrix}~A}\Phi^\top \theta + \eta \Phi \sum_{i=1}^n D^{\pi_i} R\quad.
\end{align}

This gradient flow is guaranteed to be asymptotically stable (meaning it will not diverge around the stationary point $\theta^*$) if the key matrix $A = \sum_{i=1}^n D^{\pi_i} \left(I - \gamma P \Pi \right)$ is positive definite (see \textcite{sutton1988learning} for a more thorough explanation). 
...
Note that when we talk about positive-definiteness, we use the definition from \textcite{sutton1988learning} which applies to potentially non-symmetric matrices, and merely requires that $u^\top X u > 0$ for all vectors $u$.
...
\begin{proof}
    ...
    Furthermore, if the diagonal entries of a symmetric matrix are positive and all its off-diagonal entries are negative, then it suffices to show that the row and column sums of matrix are positive, as this is a sufficient condition to show that the matrix is positive diagonal-dominant.
...
The case we have discussed previously, where actions miss completely, is an extreme version of this, but even if we have samples of all state-action pairs, the distribution shift between the behavior and target policy can lead to divergence.
...
The architecture is presented in detail in \autoref{sec:mad:method}, and is regularized to prevent catastrophic divergence\footnote{Here we use the strategy proposed by \textcite{lavoie2023simplicial} instead of the projection proposed in \autoref{chap:overestimation}, as this proved to be amenable to more stable model learning losses using cross-entropy minimization instead of mean squared error for the latent model loss.}, uses latent self-prediction to stabilize feature learning (compare \autoref{chap:understanding}), and uses clipped double Q learning \parencite{fujimoto2018addressing}.
...
This phenomenon was previously discussed in the context of over-training on limited data in \autoref{chap:overestimation}.
...
However, as our experiment presented in\autoref{sec:overestimation:limitations_experiment} and \autoref{sec:mad:off-policy-eval-exp} highlight, by themselves they can mitigate catastrophic overestimation and divergence, but do not guarantee proper generalization. %
...
However, in complex settings this assumption on the error of the critic estimates may not hold. 
...
Our approach builds on the TD3 algorithm \parencite{fujimoto2018addressing} and the TD-MPC2 architecture and model \parencite{hansen2024tdmpc}. 
...
To see this, note that the goal of our policy optimization is to find $\max_a Q(x,a)$. 
A potential adversarial objective for a regression problem would attempt to find $\delta$ so that 
\begin{align*}
    &\max (Q(x,a+\delta) - Q(x,a))^2\\
    \mathrm{s.t.}~&\|\delta\|_2 \leq \epsilon
\end{align*}
Note that while adversarial examples are constrained to not deviate far from the reference sample, our off-policy policy improvement objectives are not restricted in a similar fashion.
Indeed, \textcite{fujimoto2018addressing} references this phenomenon while discussing the clipped Q learning trick, but as we have seen throughout several chapter now, clipping alone cannot prevent persistent Q function overestimation.
However, we conjecture that the additional samples provided in model-based reinforcement learning can smooth out the Q landscape with regards to a, so that finding a policy that strongly and erroneously overestimates Q becomes harder.
We provide evidence for this conjecture in \autoref{sec:mad:adv_robustness}.
...
Following the evidence presented in \autoref{chap:cvaml}, we use the latent VAML loss instead of the MuZero loss used by \textcite{hansen2024tdmpc}.
In addition, since we have a categorical Q representation, we use a cross-entropy loss to compute the VAML term instead of a squared loss. 

\textbf{Full loss:}~~~
With these changes, the full model and Q learning loss can be written as follows.
As we use the HL-Gauss representation \parencite{farebrother2024stop} for the critic, the loss is the cross-entropy between the estimated Q function's categorical representation $Q_\mathrm{rep}$ and the bootstrapped TD estimate,
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/mad-td/critic_loss_dog_walk_model_based_separate.pdf}
...
\begin{figure}[t]
    ...
    \caption{Return curves for the dog tasks with differing UTD values. Across all environments, the return increases or remains stable with increasing UTD with MAD-TD. Without model data, the performance decreases when increasing the UTD. MPC is turned off in these runs to cleanly evaluate the impact of model data on critic learning.}
...
\begin{figure}[!b]
    ...
    \caption{Performance comparison on the hard tasks for \sbcblue{MAD-TD}, \sbcorange{BroNet}, and \sbcgreen{TD-MPC}, with varying number of steps and action repeat settings. MAD-TD is on par with all baselines, has higher mean and IQM when trained for 2 million time steps and action repeat 2, and strongly outperforms TD-MPC2 and BroNet at 1 million time steps with action repeat 2.}
...
We find that our method performs on par or above previous methods, and strikingly it is able to achieve higher returns faster than both TD-MPC2 and BroNet.
...
\begin{figure}[t]
    ...
    \caption{Resetting evaluation of \sbcblue{MAD-TD} and \sbcgreen{BroNet}. Lighter color denotes performance with reset, and darker without. While MAD-TD's performance only increases slightly when adding resetting, BroNet is unable to achieve strong performance in and setting without resetting.}
    \label{fig:mad:comp_reset}
\end{figure}

\subsection{Performance and stability impact of resetting}
\textbf{Resetting comparison:}~~~To investigate whether our technique benefits from more stable training, we set up a comparison in which we test the effects of resetting on our method. \autoref{fig:mad:comp_reset} presents aggregate results comparing our approach and BroNet, both with and without resetting. 
...
However, the BroNet algorithm is not able to achieve reliable performance without resets.
...
\begin{wrapfigure}[]{O}[0pt]{0.4\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figures/mad-td/higher_data_aggregate.pdf}
...
\paragraph{\rebuttal{SynthER}} A {related} approach to obtain additional data is the diffusion-based method proposed by \textcite{lu2024synthetic}.
...
\textbf{Perturbation robustness of the model-corrected value function:}~~~In the introduction of our method, we conjectured that one of the problems of training in actor-critic learning is that the actor is conducting a quasi adversarial search for overestimated values on the learned critic.\footnote{\emph{Quasi} because the actor is not constrained to find an action close to the replay buffer sample.}
To understand how easy it is to find strongly overestimated Q values with gradient-based policy search procedure used in methods like TD3 \parencite{fujimoto2018addressing} or SAC \parencite{sac}, we conduct a search for adversarial action examples on our replay buffer.
We used the iterated projected gradient method \parencite{madry2018towards} to find actions which lead to strongly increasing Q values close to the replay buffer reference actions across training on the humanoid environments at a UTD of 1 with and without model data.
...
This design choice proved to be beneficial but would require learning an observation-space next-state prediction, which is difficult in practice, especially in high dimensional environments.
...
\textcite{silver2017predictron} proposes a latent model that is trained with TD learning, which provides the basis for the MuZero algorithm \parencite{schrittwieser2020mastering}.
...
In the context of batch reinforcement learning or offline RL \parencite{lange2012batch,fujimoto2019bcq}, the action distribution shift is a known phenomenon.
The main counter to the problem however does not rely on closing the generalization gap, but on explicit pessimistic regularization \parencite{jin2021pessimism}.
Such pessimistic regularization has been shown to be highly detrimental in online RL, as it removes the capability for the agent to explore its environment efficiently \parencite{doro2023barrier,hussing2024dissecting}.
In offline RL, authors have explored the capability of models to provide some improvements to generalization \parencite{yu2020mopo}. 
...
A phenomenon that has been observed in both the continual learning and RL communities is the tendency for neural network based agents to lose their ability to learn over time (see \textcite{parisi2019continual} for a survey of continual learning and \textcite{igl2021transient} for a discussion of the phenomenon in RL). Sometimes the phenomenon is referred to as plasticity loss~\parencite{lyle2022understanding, abbas2023loss,dohare2024loss}. 
...
Previous studies have identified and combated plasticity loss using feature rank maximization \parencite{kumar2021implicit}, regularization \parencite{lyle2023understanding}, additional neural network copies \parencite{nikishin2024deep}, minimizing dormant neurons \parencite{sokar2023dormant, xu2024drm}, various neural network architecture changes \parencite{lee2023plastic}, slow and fast network updates \parencite{lee2024slow} or weight clipping \parencite{elsayed2024weightclipping}. 

It is unclear how many improvements obtained by these changes can be explained by divergence effects (\autoref{chap:overestimation}) or stability issues such as those established in our work as there seems to be a non-zero overlap in techniques that combat either. 
...
Here we will give a non extensive overview into the different directions that exist as a starting point for the reader. For instance from a theoretical perspective, stability can be formulated as differential privacy \parencite{vietri2020private} or algorithmic replicability to obtain identical policies \parencite{eaton2023replicable}. From a theoretical as well as practical perspective, stability to changes in the environment can also be investigated using random or adversarial perturbations to the transition model or the policy \parencite{nilim2005robust, iyengar2005robust, wiesemann2013robust, pinto2017robust}. Finally, from an empirical perspective, robustness to hyperparameters \parencite{ceron2024consistency, patterson2024cross} and attempts at variance reduction to get more reliable solutions \parencite{anschel2017averaged, kuang2023variance} can be considered notions of stability.
