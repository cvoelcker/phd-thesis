...
Especially for achieving efficient learning, throwing away all learned parameters and starting from scratch periodically is counter-productive, for instance in scenarios where, keeping all previous experience is infeasible. %for instance when data buffer sizes become limited. 
...
Even after a large amount of priming steps, the agent immediately starts to learn. %, rendering resetting unnecessary. 
...
% The results in~\ref{fig:overestimationpriming} indicate that a key property of the training runs is that with increased $U$, 
...
%     \label{fig:overestimationhopper}
% \end{wrapfigure}
...
% \textcite{lyle2022understanding} present an alternative regularization to ours which aims to keep the parameters of the network close to initialization.
