...
To improve sample efficiency in off-policy, value-based \ac{rl} contemporary work has begun increasing the number of gradient updates per collected environment step~\parencite{janner2019mbpo,fedus2020revisiting,chen2021randomized, hiraoka2022dropout, nikishin2022primacy, doro2023barrier, schwarzer2023bigger, kim2023resetensemble}.  
However, as this update-to-data (UTD) ratio increases, value function learning seems to become unstable.
This is perhaps surprising, as neural networks have shown themselves to be generally robust to the number of update steps in other areas such as supervised classification.
In many cases, training for a very large number of steps actually increases the performance of neural networks, due to phenomena such as grokking \parencite{power2022grokking} or double descent \parencite{belkin2019reconciling}.
...
Resetting network parameters is a useful technique that, in some sense, can circumvent any ongoing optimization failures. 
...
data. However, contrary to prior work, we find that it is not \emph{impossible} to learn even after priming, it merely takes a long time and many samples to start achieving non-trivial returns. 
...
Both weight decay as well as dropout can somewhat reduce the divergence during priming, but not to a sufficient degree to allow for fast learning afterwards.
While we completely fail to attain strong performance with weight decay, dropout is able to recover a good amount of final return.
...
We therefore conjecture that divergence is not solely a consequence of the amount of experience gathered but rather happens due to state-action space coverage. 
We will revisit and test this conjecture in \autoref{chap:mad}.
Note that the reported Q-values have been measured on the training data, not on a separate validation set. 
...
We therefore expect the average Q-values to be roughly at $10\%$ of the undiscounted return which is roughly achieved by the OFN network. 
...
To corroborate this conjecture, we repeated some experiments with a single critic. As OFN handles divergence it might not require a minimization over two critics~\parencite{fujimoto2018addressing}. We compare OFN using a single critic and $32$ updates per environment step to standard SAC and OFN in Figure~\ref{fig:overestimation:hopper_hop}. With a single critic, OFN does not get stuck in a local minimum and outperforms full resetting. Note that this is only true in few environments, leading us to believe that the effects of high-update training are at least in part MDP-dependent.
...
\label{sec:overestimation:limitations_experiment}
...
To answer \textbf{Q3}, we move to a set of training environments which are considered especially hard for model-free approaches, namely the dog tasks of the DMC suite. Standard SAC can generally not obtain any reasonable reward and, due to their complex dynamics, these tasks are often tackled using model-based approaches such as TD-MPC2~\parencite{hansen2024tdmpc} with complicated update procedures and carefully tuned network architectures. We evaluate SAC and OFN on the dog tasks and compare against TD-MPC2 in Figure~\ref{fig:overestimation:all_dog}.
...
Overestimation in off-policy value function learning is a well-established problem.
Discussions in the RL literature date back far before the prime times of deep learning~\parencite{thrun1993issues, precup2001off}. 
