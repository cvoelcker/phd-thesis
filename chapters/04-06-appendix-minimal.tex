...
The reference implementation is provided under \href{https://github.com/adaptive-agents-lab/MAD-TD}>
...
For several experiments, we only showed the dog results from the main suite to avoid cluttering the main body of the thesis.
...
As described in \autoref{chap:mad}, we simplify the base model of TD-MPC2 to improve the computational efficiency of the algorithm.
...
As pointed out, all of our changes to the base model boil down to setting different hyperparameters, such as the rollout length, to achieve faster learning.
...
These are presented in \autoref{fig:mad:mpc_comp} and \autoref{fig:mad:all_baseline}.
