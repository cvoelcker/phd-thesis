...
The formulation of VAML which we derive for SAC is a direct extension of the VAML loss to the SAC soft Bellman backup. Specifically, given some distribution $\mathcal{D}$ over the state-action space, state-action value function $Q$ and policy $\pi$, we can define a SAC-aware loss.

\begin{align}
    &\mathcal{L}_{Q, \pi}(\hat{p}, p, \mathcal{D}) = \nonumber\\
    &\quad \int \left|\int \left(\hat{p}(x'|s,a) - p(x'|s,a)\right) \E_{a' \sim \pi(\cdot| x')}{\left(Q(x', a') - \log \pi(a'|x')\right)} \mathrm{d}x'\right|^2 \mathrm{d} \mathcal{D}(x,a)\\
    &= \int \bigg|\int \left(\hat{p}(x'|x,a) - p(x'|x,a)\right) V^\pi(x', a')\mathrm{d}x' \nonumber\\
    &\quad\quad\quad- \int \left(\hat{p}(x'|x,a) - p(x'|x,a)\right)\mathbb{E}_{a' \sim \pi(\cdot| x')}\left[\log \pi(a'|x')\right] \mathrm{d}x'\bigg|^2 \mathrm{d} \mathcal{D}(x,a)
\end{align}
as well as its sample-based version:
\begin{align}
    &\hat{\mathcal{L}}_{Q, \pi}(\hat{p}, p, \mathcal{D}) = \sum_{i} \bigg|V^\pi(x_i') -\int \hat{p}(x'|x_i, a_i) V^\pi(x') \mathrm{d}x' - \nonumber\\&\quad\quad\left(\int \pi(a'|x_i')\log\pi(a' | x_i') \mathrm{d}a'- \int \int \hat{p}(x'|x_i,a_i)\pi(a'|x')\log\pi(a' | x')\mathrm{d}a' \mathrm{d}x' \right)\bigg|^2\label{SACVAMLLoss}
\end{align}
...
As described in \autoref{chap:vagram}, we conducted our experiment in two domains from the OpenAI benchmark OpenAI Gym \parencite{brockman2016openai}, Pendulum-v2 and Hopper-v0.
...
To test the performance of VaGraM in a setting with additional unnecessary state observations, we created a random dynamical system that evolves according to randomly initialized dynamics function, where $A$ is a fixed matrix with entries randomly drawn from $\mathcal{N}(0, 10)$, and a fixed initial state $s$ with components randomly drawn from $\mathcal{N}(0, 0.1)$. We do not resample the fixed components at reset.
...
\begin{align}
    f(x_t,a_t) &= 
  \begin{cases}
    x_t + \sin(A x) & \text{if } |f(x_t,a_t)| < 20\\
    x_0 & \text{else}
  \end{cases}
\end{align}
...
The exact versions of all used libraries are documented in the source code\footnote{\url{https://github.com/pairlab/vagram/tree/main}}.
A modern re-implementation using the jax library can be found under \url{https://github.com/cvoelcker/jax_mbpo}.
REsults differ slightly from those presented here due to library and environment version differences.
...
To estimate the gradient of the value function, we used four empirical value functions, two direct estimates and two target value functions used in the SAC algorithm and summed the losses using each data tuple and value function gradient independently.
...
\For{$N$ epochs}
{
    \While{$\hat{p}_\theta$ not converged}{
        Sample batch $(x,a,r,s')$ from $\mathcal{D}_\text{env}$\;
        $\mathcal{L}_{v_\psi} = \left((x' - \hat{f}_\theta(x,a))^\top \text{diag}\left(\frac{d}{ds}v_\psi(x)|_{x'}\right)^2 (x' - \hat{f}_\theta(x,a))\right)$\;
    ...
    \For{M model rollouts}{
        ...
        $s', r = \hat{f}_\theta(x,a)$\;
        Add $(x,a,r,x')$ to $\mathcal{D}_\text{model}$
    }
    \For{G policy gradient updates}{
        Sample batch $(s,a,r,s')$ from $\mathcal{D}_\text{env} \cup \mathcal{D}_\text{model}$\;
        $\psi \gets \psi - \beta \frac{d}{d\psi} (v_\psi(x) - (r + \gamma v'(x')))^2$\;
        $\phi \gets \phi - \lambda \hat{\nabla}_\phi J(\pi_\phi, v_\psi, (x,a,r,x'))$
    }
    }
}
\end{algorithm}
...
Nonetheless, VaGraM is still consistently able to achieve higher cumulative reward than the MSE model. 
