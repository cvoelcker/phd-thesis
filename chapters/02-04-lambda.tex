...
As discussed in the previous chapter, the paradigm of \emph{value-aware model learning} (VAML) \parencite{vaml} and \emph{value-equivalence}~\parencite{grimm2020value,grimm2021proper} addresses this by training models that lead to accurate value estimation.
...
In the previous chapter however we saw that empirically, IterVAML approaches tend to be unstable when used with observation-space models.
However, as we discuss previous experiments with IterVAML and MuZero differ significantly in terms of the neural network architectures used in practice.

In this chapter we show that MuZero and IterVAML can be unified in a family of losses, which we term $(m,b)$-Value-Aware Model Losses ($(m,b)$-VAML).
...
\textbf{Research question:} This chapter has three parts, each with a theoretical and empirical section. We answer two questions about the $(m,b)$-VAML family: 
\begin{enumerate}[(a)]
    \item What variants of the $(m,b)$-VAML losses are well-calibrated to recover correct models and value functions?
    \item How do deterministic latent-model architectures impact the stability of $(m,b)$-VAML algorithms?
    \item Do we observe problems with uncalibrated losses when using stochastic architectures?
\end{enumerate}

\textbf{Contributions:} 
As our main theoretical contribution, we mathematically analyze the family of $(m,b)$-VAML algorithms.
We prove that all sample-based members of this loss family are uncalibrated when used with a stochastic environment model.
...
The value is the unique fixed point of the Bellman operator $$[\mathcal{T}_{\P^\pi} V](x) = \EEX{\pi}{r(x_0,a_0) + \gamma  \EEX{\P^\pi}{V(x_1)}|x_0=x}.$$
...
We will use $\hat{x}^{(n)}$ to refer to samples from the $n$-th step model prediction and write $\EEX{\hat{p}^{(m)}}{\cdot}$ as a shorthand for $\EEX{{x}^{(m)}\sim \hat{p}(\cdot|x)}{\cdot}$.
...
\begin{figure*}[t]
    ...
    \begin{align*}
        ...
        = &\biggl|{\color{newgreendeal}\underbrace{\hat{V}\roundb{\hat{x}^{(m)}}}_\mathrm{model}} - 
...
The loss function introduced previously can be seen as a single-step and expectation-based variant of this more general loss.
...
\begin{aligned}
     &{\mathcal{L}}^{\P^\pi}_{\mathrm{IterVAML}, m}\left(\hat{p},V|x\right) = \biggl|\mathbb{E}_{\hat{p}^m} \squareb{V\roundb{\hat{x}^{(m)}}} - 
    \squareb{\EEX{\P^\pi}{V\roundb{x^{(m)}}}}_\mathrm{sg}\biggr|^2 \\
    & {\mathcal{L}}^{\P^\pi}_{\mathrm{IterVAML}, m}\left(\hat{p},V|x\right) \approx \hat{{\mathcal{L}}}^K_{\mathrm{IterVAML}, m}\left(\hat{p},V|x, x^{(m)}\right) = \biggl|\frac{1}{K}\sum_{k=1}^K \squareb{V\roundb{\hat{x}_k^{(m)}}} - 
...
The set of models that achieve $0$ IterVAML error is equal to the set of value-equivalent models for the value estimate $\hat{V}$ \parencite{grimm2021proper}.
...
Note that samples from the real environment are used to approximate $[\mathcal{T}_{\P^\pi}^b \hat{V}]$.
...
In addition, $(m,b\geq1)$-VAML, MuZero, is used to update the value function directly in addition to the model.
...
Formally, we require that the surrogate loss is not perfectly minimized by a function that does not perfectly minimize the target loss.
Therefore, we call losses that do not have this property \emph{minimum-uncalibrated} losses, or simply uncalibrated outside of formal statements.
We borrow the concept from \textcite{steinwart2008support}, however, we use a slightly more restricted definition of \emph{uncalibrated} here.
This means that all losses which we consider uncalibrated are also uncalibrated in the definition given in \textcite{steinwart2008support}, but not vice versa.


\begin{definition}[Minimum-uncalibrated surrogate losses]
    ...
    Let $\mathcal{F}^*$ be a set of (perfect) minima of $\mathcal{L}_\mathrm{tar}$ so that $\mathcal{L}_\mathrm{tar}(f^*,x,x^{(m)}) = 0$ for all $f^* \in \mathcal{F}^*.$
    A surrogate function is \emph{minimum-uncalibrated} for the target loss if there exists an \ac{mdp}, a function class $\mathcal{F}$ with $\mathcal{F}^* \subseteq \mathcal{F}$, and a state $x$ so that 
    $$\argmin_{f \in \mathcal{F}} \mathbb{E}_{x^{(m)}\sim\P^\pi(\cdot|x)}\left[\mathcal{L}_\mathrm{sur}(f,x,x^{(m)})\right] \not\subseteq \mathcal{F}^*.$$
\end{definition}
A prominent example of an minimum-uncalibrated loss is \ac{brm}, which results in the \emph{double sampling issue}.
...
\begin{table*}
    ...
    \begin{center}
        \begin{tabular}{l | l|c|c|c}
            ...
            Model & Env & {$(m\geq1,b=0)$} &  $(m\geq1,b\geq1)$& $(m=0,b\geq1)$\\\hline
...
To show this for a general class of MDPs, we require the existence of at least one MPD with transition function that fulfils the following condition.
\begin{assumption}\label{ass:cvaml_mdp}
Let $p$ be a probability distribution over $\mathcal{X}$, and $f: \mathcal{X} \rightarrow \mathbb{R}$, so that no $x$ exists for which $f(x) = \EEX{p}{f(x)}$.
\end{assumption}
The existence of such an MDP is trivial to show.
Take, for example, a two state single action MDP with a transition matrix and function of $$\mathcal{P}=\begin{bmatrix}
0.5 & 0.5\\
0.5 & 0.5 
\end{bmatrix}, f=\begin{bmatrix}
    ...
    1
\end{bmatrix}.$$
Take $p$ in the assumption to be $\mathcal{P}(\cdot|x_1)$. In this case, $\mathbb{E}_p[f(x)] = 0.5$, but no state $x$ exists so that $f(x) = 0.5.$
Therefore, the assumption is justified by example.

This is an important assumption as it guarantees that there is no distribution $q$ with $0$ variance such that $\EEX{q}{f(x)} = \EEX{p}{f(x)},$ which would result in a ``trivial'' deterministic IterVAML error.
...
The assumption is used to prove the existence of a ``better'' solution under the surrogate sample-based loss that does not minimize the IterVAML target loss.

We will revisit this assumption in \autoref{sec:cvaml:theory_2}, where we will relax the assumption on $f$ to show that if we can embed our states $x$ into a continuous representation space (under some additional technical conditions) a deterministic model with 0 IterVAML loss always exists.

We begin with analyzing the loss for $m\geq1$ and $b=0$.
\begin{restatable}{proposition}{IterVAMLUncal}\label{prop:cvaml:2_1}
    Let $\hat{\mathcal{L}}^{\P^\pi}_{m\geq1,0}(\hat{p},V|x,x^{(m)})$ be a surrogate loss for $\mathcal{L}_{\mathrm{IterVAML}, m}^{\P^\pi}(\hat{p}, V|x)$.
    ...
    Therefore, $\hat{\mathcal{L}}_\mathrm{model}$ is \emph{minimum-uncalibrated}.
\end{restatable}
\begin{proof}
Expanding the empirical IterVAML loss with $k$ samples, obtain
\begin{align}
   & \EEX{\P^\pi}{\hat{\mathcal{L}}^{\P^\pi}_{m\geq1,0}\left(\hat{p},V|x,x^{(m)}\right)} \\
...
The first term is the variance of the value under the model. 
The final term can again be decomposed as
\begin{align}
   &(2) = \EEX{\hat{p},\P^\pi}{\left(\EEX{\hat{p}}{V\left(\hat{x}^{(m)}\right)} - V\left(x^{(m)}\right)\right)^2} \\
...
When we drop the term that is independent of the model, the loss is composed of the target loss, and an additional variance term.
\begin{align}
   & \EEX{\P^\pi}{\hat{\mathcal{L}}^{\P^\pi}_{i\geq1,0}\left(\hat{p},V|x,x^{(m)}\right)} \\
   \quad & =  \underbrace{\EEX{\hat{p},\P^\pi}{\left(V\left(\hat{x}^{(m)}\right) - \EEX{\hat{p}}{V\left(\hat{x}^{(m)}\right)}\right)^2}}_{= \mathrm{Var}} \nonumber \\
   \quad & \quad +\underbrace{\EEX{\hat{p},\P^\pi}{\left(\EEX{\hat{p}}{V\left(\hat{x}^{(m)}\right)} - \EEX{\P^\pi}{V\left(x^{(m)}\right)}\right)^2}}_{= \mathrm{IterVAML}} \label{eq:final_itervaml_error}
\end{align}

We prove in \autoref{lemma:cvaml:5} that there exist problems in which the ground truth model does not minimize this loss.
...
Consequently, for larger values of $k$, the condition in \autoref{lemma:cvaml:4} for $g$ becomes stricter and the calibration error decreases.
As $k \rightarrow \infty$, the condition becomes unfulfillable. 
This is also intuitive, as $$\frac{1}{k}\sum_{i=1}^k V\left(x^{(m)}_i\right) \rightarrow \EEX{\hat{p}}{V\left(x^{(m)}\right)}$$ as $k \rightarrow \infty$ almost surely (assuming standard conditions hold).
...
To obtain the correction, we define $\hat{\mu}_{\hat{p}}^{m,k} = \frac{1}{k}\sum_{i=1}^k V(\hat{x}_i^{(m)})$, the empirical estimator of the expected return in \autoref{eq:cvaml:itervaml}.
The variance of this estimator can be estimated as $\widehat{\mathrm{Var}}_{\hat{p}}^{m,k} = \frac{1}{k} \sum_{i=1}^{k} (V(\hat{x}_k^{(m)}) - \mu_{\hat{p}}^{m,k})^2$.
With this we can define a new loss which can be computed with at least two samples from the model
$$\hat{\mathcal{L}}_{\mathrm{CVAML}, m}^{k} = \hat{\mathcal{L}}^k_{\mathrm{IterVAML}, m} - \widehat{\mathrm{Var}}_{\hat{p}}^{i,k}.$$
...
\begin{restatable}{proposition}{IterVAMLVar}\label{prop:cvaml:2_2}
    ...
    The variance-corrected loss $\hat{\mathcal{L}}_{\mathrm{CVAML}, m}^{k}(\hat{P}, V|x, x^{(m)})$ is a calibrated surrogate loss for $\mathcal{L}_{\mathrm{IterVAML}, m}^{\P^\pi}(\hat{P}, V|x, x^{(m)})$.
\end{restatable}
\begin{proof}
Reusing the previous derivation in the proof of \autoref{prop:cvaml:2_1}, \autoref{eq:final_itervaml_error}, we obtain
\begin{align}
    & \EEX{\P^\pi}{\hat{\mathcal{L}}_{\mathrm{var}, m}^{k}(\hat{P}, V|x, x^{(m)})} \\
    = &\EEX{\P^\pi}{\hat{\mathcal{L}}^{\P^\pi}_{i\geq1,0}\left(\hat{p},V|x,x^{(m)}\right)} - \frac{1}{k}\underbrace{\EEX{\hat{x},\P^\pi}{\left(V\left(\hat{x}^{(m)}\right) - \EEX{\hat{p}}{V\left(\hat{x}^{(m)}\right)}\right)^2}}_{= \mathrm{Var}}\\
...
As the target loss and the surrogate loss are equivalent in expectation, all minimizer of the expected CVAML loss minimize the target loss, which fulfills the requirement for calibration.
\end{proof}

Analogous to the IterVAML case, the sample-based $(m,b \geq 1)$-VAML loss is an uncalibrated loss for learning a model.
To address this issue, we will introduce a corresponding variance correction term.
...
\end{align}
Note that the issue arises because we are estimating values of successor states with the MuZero loss.
Therefore, while the loss depends on the state $x$, all computation is actually conducted wrt the sampled successor states $x^{(m)}$.

\begin{restatable}{proposition}{MuZeroValue}\label{prop:cvaml:2_3}
    Let $\mathcal{L}_\mathrm{TD}(V|V_\mathrm{tar}, x^{(m)})$ be the target loss, and let $\hat{\mathcal{L}}^{\P^\pi}_{m,1}(\P^\pi,V|\allowbreak V_\mathrm{tar},x, x^{(m), r^{(m)}})$ be the surrogate loss. 
    There exists a set of functions $\mathcal{V}$ for any $V_\mathrm{tar}$ that is not a constant function, for which two conditions hold:
    \begin{enumerate}
        \item The set is complete, meaning that $[\mathcal{T}_{\mathcal{P}^\pi}V_\mathrm{tar}] \in \mathcal{V}$ for some target function $V$,
        \item and,
    \end{enumerate}
     $$ \Argmin_{\hat{V} \in \mathcal{V}} \mathbb{E}_{\P^\pi}\squareb{\hat{\mathcal{L}}^{\P^\pi}_{m,1}(\P^\pi, \hat{V}| V_\mathrm{tar}, x, x^{(m)})} \not \subseteq [\mathcal{T}_{\P^\pi} V_\mathrm{tar}].$$

    Therefore, $\hat{\mathcal{L}}^{\P^\pi}_{m,1}$ is minimum-uncalibrated.
\end{restatable}
\begin{proof}
...
The first term (\autoref{eq:first}) is the bootstrapped Bellman residual with a target $V_\mathrm{tar}$.
...
To overcome this issue, we will introduce another variance correction term, similar to $\hat{\mathcal{L}}_\mathrm{CVAML}$. 
...
However, these are necessary for accurate planning or policy improvement.
Therefore, it is still important to use a loss variant with $m=0$ for learning accurate values for each state.
...
Every problem is generated by sampling $m$ successor states for each state $x_i$, and parameterizing the transitions with weights $\omega_{i,j}$ sampled uniformly iid for all successor states $x_j$.
The transition distribution is given by $$p(x'_i|x_j) = \frac{e^{\omega_{i,j}/\tau}}{\sum_i e^{\omega_{i,j}/\tau}}.$$
...
For the near-deterministic ground truth environment, we see no benefit from using a calibrated $(m,b)$-VAML.
...
In practice, most implementations of MuZero-based models use deterministic latent model structures \parencite{schrittwieser2020mastering,ye2021mastering,hansen2022temporal,antonoglou2022planning}, while our investigation of IterVAML in the previous chapter, as well as similar experiments in \textcite{lovatto2020decision}, were conducted with observation-space models.
...
These are standard assumptions that are necessary to prove the measurability of many relevant functions in continuous state-action spaces
For formal definitions, especially the definitions on the continuous stochastic kernels, see \textcite[p.134]{bertsekasshreve1978}. 
...
Since $\phi$ is a bijective continuous function over a compact space and maps to a Hausdorff space (by assumption $\mathcal{Z}$ is metrizable, which implies it is a Hausdorff space), it is a homeomorphism.
...
Then, $\theta_{V,\mathcal{X}}$ is continuous (\autoref{prop:cvaml:730}).
...
For any $z \in \mathcal{Z}_\mathcal{X}$, by the intermediate value argument, there exists $z'$ such that $V(z') = V(z)$. 
...
By \autoref{prop:cvaml:733}, there exists a measurable function $f^*(z)$ so that $\min_{z'} h(z, z') = h(z, f^*(z)) = 0$.
...
\begin{restatable}{proposition}{DeterministicModel}\label{prop:cvaml:3_1}
    ...
    Furthermore, the same $f^*$ is a minimizer of the expected IterVAML loss over any distribution $x \sim \rho$
    $$f^* \in \argmin_{\hat{f}} \EEX{\rho,\P^\pi}{\hat{\mathcal{L}}_{\mathrm{IterVAML},1}(\hat{f},V_\mathcal{X}|V_\mathcal{X},x)}.$$
\end{restatable}
...
\begin{align}
     &\EEX{\rho,\P^\pi}{\hat{\mathcal{L}}_{\text{IterVAML}, 1}(f, V | V_\mathcal{X} x, x' } \\
    &\quad = \EEX{\rho,\P^\pi}{\squareb{V\roundb{f\roundb{\phi(x)}} - V_\mathcal{X}(x')}^2}\\
    &\quad = \EEX{\rho,\P^\pi}{\Bigl(V\roundb{f\roundb{\phi(x)}} - \EEX{\P^\pi}{V_\mathcal{X}(x^{(1)})} + \EEX{\P^\pi}{V_\mathcal{X}(x')} - V_\mathcal{X}(x^{(n)}) \Bigr)^2}.
\end{align}
After expanding the square, we obtain three terms:
\begin{align}
    & \EEX{\rho,\hat{p},\P^\pi}{\hat{\mathcal{L}}_{\text{IterVAML}, 1}(f, V | V_\mathcal{X} x, x') } \\
    = &\EEX{\rho,\hat{p},\P^\pi}{\left|{V\roundb{f\roundb{\phi(x)}} - \EEX{\P^\pi}{V_\mathcal{X}(x')}}\right|^2} \\
    &\quad + 2 \EEX{\P^\pi}{\squareb{V\roundb{f\roundb{\phi(x)}} - \EEX{\P^\pi}{V_\mathcal{X}(x')}}\squareb{\EEX{\P^\pi}{V_\mathcal{X}(x')} - V_\mathcal{X}(x') }}\\
    &\quad + \EEX{\rho,\P^\pi}{\left|{\EEX{\P^\pi}{V_\mathcal{X}(x')} - V_\mathcal{X}(x') }\right|^2}
\end{align}

Apply the tower property to the inner term to obtain: 
\begin{align}
&2\EEX{\rho,\P^\pi}{\squareb{V\roundb{f\roundb{\phi(x)}} - \EEX{\P^\pi}{V_\mathcal{X}(x')}}\squareb{\EEX{\P^\pi}{V_\mathcal{X}(x')} - V_\mathcal{X}(x') }}\\
&\quad =2\EEX{\rho,\P^\pi}{\squareb{V\roundb{f\roundb{\phi(x)}} - \EEX{}{V_\mathcal{X}(x')}}\underbrace{\EEX{\P^\pi}{\EEX{\P^\pi}{V_\mathcal{X}(x')} - V_\mathcal{X}(x') \middle|x'}}_{=0}} = 0.
\end{align}
...
\begin{align}
    \argmin_f~ & \EEX{\rho,\P^\pi}{\left|{V\roundb{f\roundb{\phi(x)}} - \EEX{\P^\pi}{V_\mathcal{X}(x')}}\right|^2} + \EEX{\P^\pi}{\left|{\EEX{\P^\pi}{V_\mathcal{X}(x')} - V_\mathcal{X}(x') }\right|^2}\\
    =~ \argmin_f~ & \EEX{\rho,\P^\pi}{\left|{V\roundb{f\roundb{\phi(x)}} - \EEX{\P^\pi}{V_\mathcal{X}(x')}}\right|^2}.
\end{align}

Finally, it is easy to notice that $V\roundb{f^*\roundb{\phi(x)}} = \EEX{\P^\pi}{V_\mathcal{X}(x')}$ for all $x$ by the definition of $f^*$.
Since the lemma holds for all $x$, we do not need to specify the distribution $\rho$ further.
...
\phi, \hat{V}|x\right) = \underbrace{\hat{\mathcal{L}}^{\P^\pi}_{m,b}\left(\hat{f},\phi, \hat{V}|x\right)}_{(m,b)\mathrm{-VAML}} + \lambda \underbrace{\biggl| \hat{f}^m(\phi(x)) - \bigl[\phi(x^{(m)}) \bigr]_{\mathrm{sg}} \biggr|^2}_\mathrm{auxiliary}.
\end{align}
...
However,  learning an embedding in which the value function is linear can be difficult in practice and may not lead to stable model predictions in complex, high-dimensional environments.
...
\begin{itemize}
    ...
    \item Auxiliary losses can be used to stabilize learning $(m,b)$-VAML models in practice, but might lead to issues when used with deterministic models and stochastic environments.
\end{itemize}
...
\begin{enumerate}
    \item Do IterVAML and MuZero perform equivalently when used with latent model architectures?
...
To conduct our experiments we use two neural network architectures for the model, a stochastic and a deterministic latent model, while keeping the rest of the training setup consistent for a clean comparison.
...
For the stochastic case, the latent models are multivariate, diagonal Gaussian distributions where mean and variance are parameterized by the latent network \parencite{pets,janner2019mbpo,paster2021blast}.
...
The auxiliary loss for stochastic models is the negative log-likelihood of the next states' latent representations under the current dynamics model,
\begin{align}
    &\mathcal{L}_{\mathrm{aux},m}\left(\hat{p},\varphi\Big|x,a^{(0:m-1)},x^{(m)}\right) =-\log \hat{p}^m\left(\varphi(x^{(m)})\Big|\varphi(x),a^{(0:m-1)}\right),
\end{align}
...
\begin{align}
    &\mathcal{L}_{\mathrm{aux},m}\left(\hat{p},\varphi\Big|x,a^{(0:m-1)},x^{(m)}\right) =\left\|\hat{p}^m\left(\cdot\Big|\varphi(x),a^{(0:m-1)}\right)-\varphi(x^{(m)})\right\|^2.
\end{align}

For training the policy and value function, we follow a data-mixing protocol.
We use both model-generated on-policy, and real environment data from the replay buffer to train a value and policy head using the $(m,b)$-VAML loss with a twinned critic parametrization similar to TD3  \parencite{fujimoto2018addressing}.
This means the model influences the value function both through the encoder and through on-policy model-generated data.
The impact of the data-mixing strategy will be discussed in further detail in \autoref{chap:mad}.

The actions for environment interactions are obtained from the actor and the environment model using MPC \parencite{hansen2022temporal}: initialized at the action produced by the actor for the current state, this algorithm iteratively refines the action to maximize the expected return.
...
For $(m,b)$-CVAML, the calibration term is computed by sampling four independent trajectories from the model ($k=4$) and calculating the means and variances of next-state values produced by the critic across the different samples.
We will refer to the uncalibrated losses as $(m,b)$-VAML for which the variance term is not added, and we choose $k=1$ to show the most pronounced effect.
...
We found the layer norms recommended by \textcite{hansen2024tdmpc} to be slightly more performant with deep models at low UTD than OFN, so we use these here and in the next chapter.
...
In \autoref{fig:cvaml:ablation_itervaml} we show the results of training $(1,0)$-CVAML and $(1,1)$-CVAML respectively without auxiliary losses.
Comparing the results to the discussion in \autoref{chap:vagram}, we see that the combination of regularized architecture and latent model structure by itself, even without auxiliary self-prediction, can lead to stable and performant models on the dog tasks.
However, on the humanoid tasks all models fail to learn without auxiliary stabilizing loss, and on the dog tasks the auxiliary task greatly increases the performance.
These results are perhaps surprising, given the results in the previous chapter and the relatively strong performance of MuZero in the literature.

\begin{figure}[t]
\centering
    \includegraphics[width=\textwidth]{figures/lambda/plts/new_abl_rew.pdf}
    \caption{Comparison of CVAML with an auxiliary task with purely decision-aware $(1,0)$-CVAML and $(1,1)$-CVAML models (no auxiliary self-prediction loss). We can see that while $(1,0)$-CVAML can learn non-trivial policies on the dog tasks, $(1,1)$-CVAML mostly fails to do so. In addition, both losses fail to learn non-trivial policies on the humanoid tasks.}
   \label{fig:cvaml:ablation_itervaml}
\end{figure}

Looking at the associated Q values in \autoref{fig:cvaml:ablation_itervaml_q} allows us to understand the difference between previous results and this one.
Training with the MuZero loss leads to greatly inflated Q values, while the IterVAML-style loss (in combination with the latent model architecture and appropriate data mixing) does not exhibit strong value divergence on the dog task.
As we are using a stochastic model architecture, $(1,1)$ also faces the additional difficulty of being trained to predict average bootstrapped Q values for future states.
In addition, here we are not training in a high-UTD regime in these experiment, while MBPO \parencite{janner2019mbpo} and by extension VaGrAM and previous investigations into IterVAML \parencite{lovatto2020decision} train with a rather aggressive UTD value of 20.
Finally, the strong performance of MuZero without auxiliary losses has been attested primarily in the Arcade Learning Environemnt \parencite{schrittwieser2020mastering}.
As our experiments in \autoref{chap:understanding} show, Atari games and continuous control tasks can show different behavior with auxiliary tasks, with the later benefitting much more strongly from their addition.

\begin{figure}[t]
\centering
    \includegraphics[width=\textwidth]{figures/lambda/plts/new_abl_q.pdf}
    \caption{Associated mean Q values over the replay buffer for CVAML with an auxiliary task and for purely decision-aware $(1,0)$-CVAML and $(1,1)$-CVAML models (no auxiliary self-prediction loss). The Q values are significantly overestimated for the $(1,1)$-CVAML loss without auxiliary task, explaining its weak performance (compare \autoref{fig:cvaml:ablation_itervaml}).}
   \label{fig:cvaml:ablation_itervaml_q}
\end{figure}

However, on the humanoid task we nonetheless see no meaningful learning, although the Q values are not diverging with either loss.
This corroborates the hypothesis of \autoref{chap:understanding} further: in the humanoid environment, intial exploration is uninformative and all Q functions are close to 0 across all seen state-action pairs.
A decision-aware loss by itself is therefore not able to learn a model that meaningfully enables the algorithm to learn meaningful state representations nonetheless.

When combined with an auxiliary loss, both loss variants learn non-trivial policies across all tasks, as the figures show.
In this combination, $(1,0)$-CVAML is consistently better or at least as good as $(1,1)$-CVAML.
...
\centering
   ...
   \caption{Results for the latent-space model experiments. We show the aggregate metrics of the final performance on dog (left) and humanoid (right) environments, following \textcite{agarwal2021deep}. Calibrating $(1,1)$-VAML leads to a clear improvement in performance in the humanoid environment, while for $(1,0)$-(C)VAML the difference is less distinct. This is consistent with our theoretical findings as learning a smaller variance model can be sufficient, but wrong value functions will be learned with a stochastic model when $b\geq1$.}
...
The difference in performance is most noticeable between the $(1,1)$-VAML and -CVAML in the humanoid task, as the uncalibrated loss impacts both model and value function learning in this case. 
This effect is less prominent with $(1,0)$-CVAML, where only the model learning is affected.
...
Finally, we observe a consistent advantage of $(1,0)$-updates over the $(1,1)$-updates.
...
Revisiting our empirical quesitons, we observe the folllowing:
\begin{enumerate}
    \item Do IterVAML and MuZero perform equivalently when used with latent model architectures? No, in fact, without additional stabilizing auxiliary loss, the IterVAML-style loss performs better than the MuZero-style loss.
    \item Do $(m,b)$-VAML algorithms improve when combined with auxiliary losses? Yes, all variants of the (C)VAML family greatly increase in performance with a latent self-prediction auxiliary loss, and in some environments, such as the humanoid tasks, the additional auxiliary loss is necessary to achieve non-trivial performance.
    \item Can we observe the effects of calibration on deterministic environments? Yes, we can observe the effects of calibration even in deterministic environments when using stochastic models. Especially with the $(1,1)$-(C)VAML loss, which is affected by calibration issues both in the model and value learning components, we observe a clear benefit from calibration. For the $(1,0)$-(C)VAML loss, calibration has a less clear impact. This is consistent with the fact that in deterministic environments, a deterministic latent model is sufficient to provide good value estimates. Therefore, the variance minimizing effect of the uncalibrated loss is less impactful on the final performance.
\end{enumerate}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/lambda/plts/agg_total.pdf} 
...
\textcite{ayoub2020model} presents an algorithm for on selecting models based on their ability to predict value function estimates and provide regret bounds with this algorithm.
...
In addition, while previous works showed that IterVAML losses can be unstable in practice \parencite{lovatto2020decision,voelcker2022value}, we find that this can be overcome by adopting the latent model architecture used in \textcite{schrittwieser2020mastering} and the auxiliary losses established by \textcite{li2023efficient,hansen2022temporal}.
When combined with a suitable value learning procedure, IterVAML performs on par with or better than MuZero in continuous control tasks.
